...
<div id="form_block_storage" class="space-y-4 pt-2">
    ...
    <div class="flex justify-end gap-2">
        <button onclick="window.printStorageDoc()" class="bg-gray-900 hover:bg-gray-800 text-white font-black text-xs px-5 py-2.5 rounded-xl shadow-xs transition">🖨️ Печать А4</button>
        <button onclick="window.generateStorageDoc()" class="bg-blue-600 hover:bg-blue-700 text-white font-black text-xs px-5 py-2.5 rounded-xl shadow-xs transition">💾 В архив (.DOC)</button>
    </div>
</div>
`;

window.getStorageHtml = (forWord = false) => {
    ...
    <div class="print-page-a4" style="font-family: 'Times New Roman', serif; font-size: 13px; color: #000; line-height: 1.5; max-width: 210mm; box-sizing: border-box;">
        ...
        <h4 align="center" style="font-size: 13px; text-transform: uppercase; margin: 0 0 25px 0; font-family: 'Times New Roman';">постановки машин на хранение</h4>
        <p align="center" style="margin-top:-20px; margin-bottom:30px; font-family: 'Times New Roman';">от ${today}</p>
        <p style="text-indent: 25px; text-align: justify; margin-bottom: 15px; font-family: 'Times New Roman';">Ответственный за хранение Инженер по ЭМТП Волчек А.А. принял на долгосрочное/сезонное хранение следующую технику: <b>${vehicle.model || '—'}</b>, инвентарный номер <b>${vehicle.inv_number || 'б/н'}</b>, гос. номер <b>${vehicle.plate || 'б/н'}</b>.</p>
        <p style="text-indent: 25px; text-align: justify; margin-bottom: 40px; font-family: 'Times New Roman';">Проверкой установлена консервация узлов и агрегатов: <b>${gost}</b>. Техника очищена от грязи, установлена на подставки, шины разгружены, аккумуляторная батарея снята.</p>
        <table style="width: 100%; border: none; margin-top: 70px;">
            <tr>
                <td style="border:none; text-align: left; font-size: 13px; font-family: 'Times New Roman';">Сдал механизатор: ________________</td>
                <td style="border:none; text-align: right; font-size: 13px; font-family: 'Times New Roman';">Принял инженер по ЭМТП: ________________ / Волчек А.А. /</td>
            </tr>
        </table>
    </div>`;
};
...
window.printStorageDoc = () => {
    ...
    printBlock.innerHTML = window.getStorageHtml(false);
    window.print();
...
window.generateStorageDoc = async () => {
    ...
    const num = document.getElementById('lifecycleNum')?.value || '1';
    const fileName = `storage_act_№${num}_${vehicle.inv_number || 'бн'}.doc`;
    const htmlContent = window.getStorageHtml(true);
