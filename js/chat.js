// js/chat.js – Полноценный чат с удалением сообщений для инженера
...
// ===== HTML-ШАБЛОН (единый стиль с дашбордом) =====
function getChatHTML() {
    return `
        <div id="chatRoot" class="fixed inset-0 z-[999] pointer-events-none">
            <!-- Плавающая кнопка -->
            <button id="chatFab" class="fixed bottom-6 right-6 pointer-events-auto bg-cyan-600 hover:bg-cyan-700 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 flex items-center justify-center w-14 h-14 backdrop-blur-sm hover:scale-105 active:scale-95">
            ...
            <!-- Окно чата -->
            <div id="chatWindow" class="fixed inset-0 md:inset-auto md:bottom-24 md:right-6 md:w-[480px] md:h-[640px] bg-white shadow-2xl border border-gray-200 pointer-events-auto flex flex-col hidden transition-all duration-300 transform scale-95 opacity-0 md:rounded-2xl overflow-hidden">
                <!-- Шапка -->
                <div class="bg-cyan-600 text-white p-4 flex justify-between items-center flex-shrink-0">
                    <div class="flex items-center gap-2">
                        ...
                        <span id="chatRoomName" class="text-xs font-medium bg-cyan-500 px-2 py-0.5 rounded-full">Общий</span>
                    </div>
                ...
                <div class="flex flex-1 overflow-hidden relative">
                    ...
                    <div id="chatSidebar" class="absolute md:relative z-20 w-64 h-full bg-gray-50 border-r border-gray-200 flex-shrink-0 overflow-y-auto transition-transform duration-300 -translate-x-full md:translate-x-0 md:block">
                        ...
                        <div class="p-2 space-y-1">
                            <button onclick="window.switchRoom('general')" class="w-full text-left px-3 py-2 rounded-lg hover:bg-cyan-50 transition flex items-center gap-2 text-sm font-medium" id="room-general">
                                <span>💬</span> Общий чат
                            </button>
...
function renderUserList() {
    ...
    container.innerHTML = filtered.map(u => {
        ...
        return `
            <button onclick="window.startPrivateChat('${u.name}')" class="w-full text-left px-3 py-2 rounded-lg hover:bg-cyan-50 transition flex items-center gap-2 text-sm ${isActive ? 'bg-cyan-100' : ''}">
                <span>${online}</span>
                <span class="flex-1 truncate font-medium">${u.name}</span>
                <span class="text-[10px] text-gray-400 bg-gray-200/70 px-1.5 py-0.5 rounded-full">${u.role}</span>
            </button>
        `;
    }).join('');
}
...
function renderMessages() {
    ...
    if (state.messages.length === 0) {
        container.innerHTML = '<div class="text-center text-gray-400 py-10 text-sm">Нет сообщений. Напишите первое!</div>';
        return;
    }
    const roleColors = {
    ...
    const currentName = state.currentUser.name;
    const canDelete = state.currentUser.role === 'Инженер по ЭМТП';
    container.innerHTML = state.messages.map(msg => {
        ...
        const deleteBtn = canDelete ? `<button onclick="window.deleteMessage('${msg.id}')" class="text-red-400 hover:text-red-600 transition text-xs ml-2">🗑️</button>` : '';
        return `
            <div class="flex items-start gap-3 ${isMe ? 'flex-row-reverse' : ''} animate-fade-in-up">
                <div class="flex-shrink-0 w-8 h-8 rounded-full bg-cyan-100 flex items-center justify-center text-cyan-700 font-bold text-sm shadow-sm">
...
window.deleteMessage = async function(messageId) {
    ...
    if (!window._supabase) return alert('Ошибка: Supabase не инициализирован');
    try {
